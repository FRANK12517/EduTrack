...
function decoratePopulationDashboard(dashboard = {}) {
  const raw = dashboard || {};
  ...
  return { ...raw, summary: { ...source, subscriptionPopulation, currentActivePopulation, newAdmissionsAfterSubscription: number(source.newAdmissionsAfterSubscription, source.newlyAdmittedPopulation, source.newly_admitted_population), withdrawalsTransfersStoppages: number(source.withdrawalsTransfersStoppages), examinationPopulation: number(source.examinationPopulation, source.examination_population), reportCardPopulation, netAdditionalStudents: netDifference, carryForwardStudents, nextSubscriptionAmountGhs: currentActivePopulation, reportCardDifference, reportCardStatus: reportCardDifference === 0 ? 'matched' : 'discrepancy', notificationState, notificationMessage, negativeDifference: Math.min(netDifference, 0) }, classes, totals, notification: { state: notificationState, message: notificationMessage, blocking: false, subscriptionPopulation, newActiveStudents: carryForwardStudents, currentActiveStudents: currentActivePopulation, carryForwardStudents, nextSubscriptionAmountGhs: currentActivePopulation }, reportCardCheck: { expected: currentActivePopulation, received: reportCardPopulation, difference: reportCardDifference, status: reportCardDifference === 0 ? 'matched' : 'discrepancy' } };
}
