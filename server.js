...
function admissionLevelForClassName(name) { const value = String(name || '').toUpperCase().replace(/\s+/g, ' ').trim(); if (/^KG\s*[0-9]/.test(value)) return 'KG'; if (/^(BASIC|CLASS)\s*[1-3]/.test(value)) return 'LOWER_PRIMARY'; if (/^(BASIC|CLASS)\s*[4-6]/.test(value)) return 'UPPER_PRIMARY'; if (/^JHS\s*[1-3]/.test(value)) return 'JHS'; return ''; }
  function assertProductionConfiguration({ requirePaystack = false } = {}) {
    if (process.env.NODE_ENV !== 'production') return;
    if (!relational.isConfigured()) throw new Error('Production requires EDUTRACK_DATABASE_URL; JSON compatibility storage is not a production persistence target.');
    if (process.env.EDUTRACK_ENABLE_DEV_ACCESS === 'true') throw new Error('Development access must be disabled in production.');
    if (!ALLOWED_ORIGINS.size || ALLOWED_ORIGINS.has('*') || [...ALLOWED_ORIGINS].some(origin => {
      try { const parsed = new URL(origin); return parsed.protocol !== 'https:' || !parsed.hostname; }
      catch { return true; }
    })) throw new Error('Production requires exact HTTPS EDUTRACK_ALLOWED_ORIGINS without wildcards.');
    if (requirePaystack && (!process.env.PAYSTACK_SECRET_KEY || !process.env.PAYSTACK_WEBHOOK_SECRET)) throw new Error('Production requires Paystack server secrets for payment operations.');
    privateStorage.validate();
  }
...
function domainErrorResponse(res, error) { if (error?.code === 'TEACHER_LIMIT') return json(res, 409, { error: error.message, teacherCount: error.teacherCount, teacherLimit: error.teacherLimit, schoolType: error.schoolType }); if (error?.code === 'ER_DUP_ENTRY') return json(res, 409, { error: 'Duplicate record' }); if (error?.code === 'ER_NO_REFERENCED_ROW_2' || error?.code === 'ER_ROW_IS_REFERENCED_2') return json(res, 400, { error: 'Referenced record is invalid' }); if (error?.message === 'INVALID_PARENT' || error?.message === 'INVALID_HIERARCHY' || error?.message === 'INVALID_ROLE') return json(res, 400, { error: 'Domain relationship is invalid' }); if (error?.code === 'DOMAIN_VALIDATION') return json(res, 400, { error: error.message }); if (error?.code === 'AUTHORIZATION_DENIED') return json(res, 403, { error: error.message }); return json(res, 500, { error: 'Domain operation failed' }); }
...
async function centralizedGovernmentTerm(context, academicYear, termNumber) {
  if (relational.isConfigured()) {
    let rows = await relational.listGovernmentAcademicCalendars(academicYear, termNumber);
    rows = rows.filter(row => ['PUBLISHED','ACTIVE','APPROVED'].includes(String(row.status || '').toUpperCase()) && new Date(row.effective_date || row.effectiveDate || 0) <= new Date()).sort((a,b) => Number(b.version || 0) - Number(a.version || 0));
    let row = rows[0];
    if (!row) {
      const legacy = await relational.domainRows("SELECT id,academic_year,term,opening_date,closing_date,status FROM academic_configurations WHERE school_id=? AND academic_year=? AND LOWER(term) IN (?,?,?) AND UPPER(status) IN ('PUBLISHED','ACTIVE','APPROVED') ORDER BY updated_at DESC LIMIT 1", [context.schoolId, academicYear, `term ${termNumber}`, `term_${termNumber}`, `term${termNumber}`]);
      row = legacy[0];
      if (row) { const durationDays = subscriptionPolicy.calculateTermDurationDays(row.opening_date, row.closing_date); return { schoolType: 'government', academicYear, termNumber, termId: String(row.id), governmentTermReference: String(row.id), governmentCalendarVersion: String(row.id), startDate: String(row.opening_date), endDate: String(row.closing_date), durationDays, source: 'legacy_centralized_academic_configuration' }; }
    }
    if (!row) throw new Error('No centrally controlled Government academic term is configured for this school and academic year');
    const durationDays = subscriptionPolicy.calculateTermDurationDays(row.reopening_date, row.vacation_date);
    return { schoolType: 'government', academicYear, termNumber, termId: String(row.id), governmentTermReference: String(row.id), governmentCalendarVersion: `${row.id}:v${row.version}`, startDate: String(row.reopening_date), endDate: String(row.vacation_date), durationDays, source: 'centralized_government_academic_calendar', calendarVersion: Number(row.version || 1) };
  }
  const centralized = (context.db?.governmentAcademicCalendars || context.db?.government_academic_calendars || []).filter(item => String(item.academicYear || item.academic_year) === academicYear && Number(item.termNumber || item.term_number) === Number(termNumber) && ['PUBLISHED','ACTIVE','APPROVED'].includes(String(item.status || '').toUpperCase()) && new Date(item.effectiveDate || item.effective_date || 0) <= new Date()).sort((a,b) => Number(b.version || 0) - Number(a.version || 0));
  let row = centralized[0];
  if (!row) { const rows = context.db?.academicConfigurations || context.db?.academic_configs || []; row = rows.find(item => String(item.schoolId || item.school_id) === context.schoolId && String(item.academicYear || item.academic_year) === academicYear && String(item.termNumber || item.term_number || item.term).replace(/[^0-9]/g, '') === String(termNumber) && ['PUBLISHED','ACTIVE','APPROVED'].includes(String(item.status || '').toUpperCase())); if (row) { const startDate = row.openingDate || row.opening_date; const endDate = row.closingDate || row.closing_date; return { schoolType: 'government', academicYear, termNumber, termId: String(row.id), governmentTermReference: String(row.id), governmentCalendarVersion: String(row.id), startDate, endDate, durationDays: subscriptionPolicy.calculateTermDurationDays(startDate, endDate), source: 'legacy_centralized_academic_configuration' }; } }
  if (!row) throw new Error('No centrally controlled Government academic term is configured for this school and academic year');
  const startDate = row.reopeningDate || row.reopening_date; const endDate = row.vacationDate || row.vacation_date;
  return { schoolType: 'government', academicYear, termNumber, termId: String(row.id), governmentTermReference: String(row.id), governmentCalendarVersion: `${row.id}:v${row.version || 1}`, startDate, endDate, durationDays: subscriptionPolicy.calculateTermDurationDays(startDate, endDate), source: 'centralized_government_academic_calendar', calendarVersion: Number(row.version || 1) };
}
...
async function privateSubscriptionCycleState(context, academicYear) {
  if (relational.isConfigured()) { const state = await relational.getSubscriptionCycleState(context.schoolId, academicYear); return { ...state, privateSubscriptionCountForAcademicYear: Number(state.privateSubscriptionCountForAcademicYear ?? state.count ?? 0) }; }
  ...
  return { count, privateSubscriptionCountForAcademicYear: count, maxSequence: rows.reduce((max, row) => Math.max(max, Number(row.subscriptionSequence || row.subscription_sequence || 0)), 0) };
}
async function validatePrivateSubscriptionWindow(context, term) {
  if (!term || !term.startDate || !term.endDate) return;
  const rows = relational.isConfigured() ? await relational.domainRows("SELECT term_number,term_start_date AS start_date,term_end_date AS end_date FROM subscriptions WHERE school_id=? AND school_type='private' AND academic_year=? AND term_start_date IS NOT NULL AND term_end_date IS NOT NULL UNION ALL SELECT term_number,term_start_date AS start_date,term_end_date AS end_date FROM payment_intents WHERE school_id=? AND school_type='private' AND academic_year=? AND term_start_date IS NOT NULL AND term_end_date IS NOT NULL AND status IN ('initialized','pending')", [context.schoolId, term.academicYear, context.schoolId, term.academicYear]) : [...(context.db?.subscriptions || []), ...(context.db?.paymentIntents || [])].filter(row => String(row.schoolId || row.school_id) === String(context.schoolId) && String(row.schoolType || row.school_type || '').toLowerCase() === 'private' && String(row.academicYear || row.academic_year) === String(term.academicYear) && ['ACTIVE','RENEWED','SUCCESS','initialized','pending'].includes(String(row.status || '').toLowerCase())).map(row => ({ term_number: row.termNumber || row.term_number, start_date: row.termStartDate || row.term_start_date || row.privateReopeningDate || row.private_reopening_date, end_date: row.termEndDate || row.term_end_date || row.privateVacationDate || row.private_vacation_date }));
  const start = new Date(`${term.startDate}T00:00:00.000Z`).getTime(); const end = new Date(`${term.endDate}T23:59:59.999Z`).getTime();
  for (const row of rows) { const existingStart = new Date(`${String(row.start_date).slice(0,10)}T00:00:00.000Z`).getTime(); const existingEnd = new Date(`${String(row.end_date).slice(0,10)}T23:59:59.999Z`).getTime(); if (Number.isFinite(existingStart) && Number.isFinite(existingEnd) && start <= existingEnd && end >= existingStart) throw new Error('Private subscription dates overlap an existing subscription period'); }
}
async function resolvePaymentContext(input, context, plan) {
  ...
  if (requestedType === 'private') {
    await validatePrivateSubscriptionWindow(context, term);
...
function decoratePopulationDashboard(dashboard = {}, schoolType) {
  const raw = dashboard || {};
  ...
  const subscriptionPopulation = number(source.subscriptionPopulation, source.subscription_population);
  const currentActivePopulation = number(source.currentActivePopulation, source.current_active_population);
  const pricing = subscriptionPolicy.calculateSubscriptionAmount(currentActivePopulation, schoolType);
  const netDifference = currentActivePopulation - subscriptionPopulation;
  const carryForwardStudents = Math.max(number(source.carryForwardStudents, source.carry_forward_students, source.netAdditionalStudents, source.net_additional_students, netDifference), 0);
  const reportCardPopulation = number(source.reportCardPopulation, source.report_card_population);
  const reportCardDifference = currentActivePopulation - reportCardPopulation;
  const notificationState = netDifference > 0 ? 'positive_difference' : netDifference < 0 ? 'negative_difference' : 'no_difference';
  const notificationMessage = netDifference > 0
    ? `${netDifference} additional active students detected.`
    : netDifference < 0
      ? `${Math.abs(netDifference)} fewer active students than at subscription.`
      : 'Your subscription population matches the current active population.';
  const classes = (raw.classes || []).map(row => {
    const subscription = number(row.subscriptionPopulation, row.subscription_population);
    const current = number(row.currentActive, row.current_active, row.currentActivePopulation, row.current_active_population);
    const exam = number(row.examInput, row.examPopulation, row.examinationPopulation, row.examination_population);
    const reports = number(row.reportCards, row.reportCardPopulation, row.report_card_population);
    const admissions = number(row.newAdmissions, row.newlyAdmittedPopulation, row.newly_admitted_population);
    const movements = number(row.withdrawalsTransfersStoppages, row.withdrawnPopulation, row.withdrawn_population) + number(row.transferredPopulation, row.transferred_population) + number(row.stoppedPopulation, row.stopped_population);
    const derivedCurrent = current || Math.max(subscription + admissions - movements, 0);
    return { ...row, class: row.class || row.className || row.class_name || row.name || row.classId || 'Unassigned', subscriptionPopulation: subscription, currentActive: derivedCurrent, examPopulation: exam, reportCards: reports, newAdmissions: admissions, withdrawalsTransfersStoppages: movements, netDifference: derivedCurrent - subscription, reportCardDifference: derivedCurrent - reports };
  });
  const totals = classes.length ? classes.reduce((acc, row) => { for (const key of ['subscriptionPopulation','currentActive','examPopulation','reportCards','newAdmissions','withdrawalsTransfersStoppages','netDifference']) acc[key] += number(row[key]); return acc; }, { subscriptionPopulation: 0, currentActive: 0, examPopulation: 0, reportCards: 0, newAdmissions: 0, withdrawalsTransfersStoppages: 0, netDifference: 0 }) : { subscriptionPopulation, currentActive: currentActivePopulation, examPopulation: number(source.examinationPopulation, source.examination_population), reportCards: reportCardPopulation, newAdmissions: number(source.newAdmissionsAfterSubscription, source.newlyAdmittedPopulation, source.newly_admitted_population), withdrawalsTransfersStoppages: number(source.withdrawalsTransfersStoppages), netDifference };
...
async function handler(req, res) {
  ...
  if (req.method === 'GET' && req.url.split('?')[0] === '/api/admin/government-academic-calendar') { const query = new URL(req.url, 'http://edutrack.local').searchParams; const academicYear = query.get('academicYear') || null; const termNumber = query.get('termNumber') ? Number(query.get('termNumber')) : null; const auth = await authorize(req, res, db, { roles: ['DEVELOPER_ROOT','SUPER_ADMIN'], permission: 'admin.summary.read' }); if (!auth) return; try { const calendars = relational.isConfigured() ? await relational.listGovernmentAcademicCalendars(academicYear, termNumber) : (db.governmentAcademicCalendars || []).filter(row => (!academicYear || String(row.academicYear || row.academic_year) === academicYear) && (!termNumber || Number(row.termNumber || row.term_number) === termNumber)).sort((a,b) => Number(b.version || 0) - Number(a.version || 0)); return json(res, 200, { calendars }); } catch (error) { return domainErrorResponse(res, error); } }
  if (req.method === 'POST' && req.url === '/api/admin/government-academic-calendar') { if (!requireSameOrigin(req, res)) return; let input; try { input = canonicalDomainPayload(await body(req)); requireFields(input, ['academicYear','reopeningDate','vacationDate']); if (input.termNumber === undefined || input.termNumber === null || input.termNumber === '') throw domainInputError('Missing required field: termNumber'); if (String(input.schoolType || 'government').toLowerCase() !== 'government') throw domainInputError('Centralized calendar applies only to Government schools'); if (!/^\d{4}\/\d{4}$/.test(String(input.academicYear)) || Number(input.academicYear.slice(5)) !== Number(input.academicYear.slice(0,4)) + 1) throw domainInputError('academicYear must contain consecutive years'); if (![1,2,3].includes(Number(input.termNumber))) throw domainInputError('termNumber must be 1, 2, or 3'); const dates = { startDate: input.reopeningDate, endDate: input.vacationDate }; subscriptionPolicy.calculateTermDurationDays(dates.startDate, dates.endDate); if (new Date(`${dates.endDate}T00:00:00.000Z`).getTime() <= new Date(`${dates.startDate}T00:00:00.000Z`).getTime()) throw domainInputError('vacationDate must be later than reopeningDate'); } catch (error) { return domainErrorResponse(res, error); } const auth = await authorize(req, res, db, { roles: ['DEVELOPER_ROOT','SUPER_ADMIN'], permission: 'admin.summary.read' }); if (!auth) return; try { const normalized = { academicYear: String(input.academicYear), termNumber: Number(input.termNumber), reopeningDate: String(input.reopeningDate), vacationDate: String(input.vacationDate), status: ['DRAFT','PUBLISHED','ACTIVE','APPROVED'].includes(String(input.status || 'PUBLISHED').toUpperCase()) ? String(input.status || 'PUBLISHED').toUpperCase() : 'PUBLISHED', effectiveDate: input.effectiveDate || input.reopeningDate }; const calendars = relational.isConfigured() ? await relational.listGovernmentAcademicCalendars(normalized.academicYear, null) : (db.governmentAcademicCalendars || []).filter(row => String(row.academicYear || row.academic_year) === normalized.academicYear); for (const prior of calendars.filter(row => Number(row.termNumber || row.term_number) !== normalized.termNumber && ['PUBLISHED','ACTIVE','APPROVED'].includes(String(row.status || '').toUpperCase()))) { const ps = new Date(`${String(prior.reopeningDate || prior.reopening_date).slice(0,10)}T00:00:00.000Z`).getTime(); const pe = new Date(`${String(prior.vacationDate || prior.vacation_date).slice(0,10)}T23:59:59.999Z`).getTime(); const ns = new Date(`${normalized.reopeningDate}T00:00:00.000Z`).getTime(); const ne = new Date(`${normalized.vacationDate}T23:59:59.999Z`).getTime(); if (ns <= pe && ne >= ps) throw Object.assign(new Error('Government academic-calendar terms may not overlap'), { code: 'ACADEMIC_CALENDAR_OVERLAP' }); } const row = relational.isConfigured() ? await relational.upsertGovernmentAcademicCalendar(normalized, auth.user.id) : (() => { db.governmentAcademicCalendars = db.governmentAcademicCalendars || []; const versions = db.governmentAcademicCalendars.filter(item => String(item.academicYear || item.academic_year) === normalized.academicYear && Number(item.termNumber || item.term_number) === normalized.termNumber); const next = { id: id('govcal'), ...normalized, schoolType: 'government', version: versions.reduce((m,item) => Math.max(m, Number(item.version || 0)), 0) + 1, createdBy: auth.user.id, updatedBy: auth.user.id, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }; db.governmentAcademicCalendars.push(next); saveDb(db); return next; })(); await auditDomainMutation(auth, 'GOVERNMENT_ACADEMIC_CALENDAR_UPDATED', req, { calendarId: row.id, academicYear: row.academicYear, termNumber: row.termNumber, version: row.version }); return json(res, 201, { calendar: row }); } catch (error) { return domainErrorResponse(res, error); } }
  if (req.method === 'GET' && req.url.split('?')[0] === '/api/subscriptions/government-calendar') { const query = new URL(req.url, 'http://edutrack.local').searchParams; const schoolId = validateText(query.get('schoolId'), { required: true, max: 80, pattern: /^[A-Za-z0-9._-]+$/ }); const academicYear = query.get('academicYear') || null; const termNumber = query.get('termNumber') ? Number(query.get('termNumber')) : null; const auth = await authorize(req, res, db, { permission: 'reporting.read', scope: { schoolId } }); if (!auth) return; const context = await authoritativeSchoolContext(auth, schoolId, db); if (!context) return json(res, 403, { error: 'Calendar school is not authorized' }); const type = subscriptionPolicy.normalizeSchoolType(context.school.ownership_type || context.school.ownershipType); if (type !== 'government') return json(res, 400, { error: 'Government calendar is only available to Government schools' }); try { const rows = relational.isConfigured() ? await relational.listGovernmentAcademicCalendars(academicYear, termNumber) : (db.governmentAcademicCalendars || []).filter(row => (!academicYear || String(row.academicYear || row.academic_year) === academicYear) && (!termNumber || Number(row.termNumber || row.term_number) === termNumber)); const active = rows.filter(row => ['PUBLISHED','ACTIVE','APPROVED'].includes(String(row.status || '').toUpperCase()) && new Date(row.effective_date || row.effectiveDate || 0) <= new Date()).sort((a,b) => Number(b.version || 0) - Number(a.version || 0)); return json(res, 200, { calendar: active[0] || null }); } catch (error) { return domainErrorResponse(res, error); } }
  if (req.method === 'GET' && req.url.split('?')[0] === '/api/admin/government-academic-calendar/dashboard') { const auth = await authorize(req, res, db, { roles: ['DEVELOPER_ROOT','SUPER_ADMIN'], permission: 'admin.summary.read' }); if (!auth) return; const query = new URL(req.url, 'http://edutrack.local').searchParams; try { const rows = relational.isConfigured() ? await relational.listGovernmentAcademicCalendars(query.get('academicYear') || null, null) : (db.governmentAcademicCalendars || []); return json(res, 200, { activeGovernmentAcademicCalendar: rows.filter(row => ['PUBLISHED','ACTIVE','APPROVED'].includes(String(row.status || '').toUpperCase()) && new Date(row.effective_date || row.effectiveDate || 0) <= new Date()).sort((a,b) => Number(b.version || 0) - Number(a.version || 0)) }); } catch (error) { return domainErrorResponse(res, error); } }
  if (req.method === 'GET' && req.url.split('?')[0] === '/api/subscriptions/private-dashboard') { const query = new URL(req.url, 'http://edutrack.local').searchParams; const schoolId = validateText(query.get('schoolId'), { required: true, max: 80, pattern: /^[A-Za-z0-9._-]+$/ }); const academicYear = validateText(query.get('academicYear'), { max: 32, pattern: /^\d{4}\/\d{4}$/ }) || null; if (!schoolId) return json(res, 400, { error: 'schoolId is required' }); const auth = await authorize(req, res, db, { permission: 'reporting.read', scope: { schoolId } }); if (!auth) return; try { let subscriptions; let intents; if (relational.isConfigured()) { subscriptions = await relational.domainRows("SELECT id,academic_year,term_number,term_start_date,term_end_date,active_student_count_at_subscription,subscription_amount,status,created_at FROM subscriptions WHERE school_id=? AND school_type='private' AND (? IS NULL OR academic_year=?) ORDER BY academic_year DESC,term_number ASC", [schoolId, academicYear, academicYear]); intents = await relational.domainRows("SELECT id,academic_year,term_number,term_start_date,term_end_date,active_student_count,subscription_amount,status,created_at FROM payment_intents WHERE school_id=? AND school_type='private' AND (? IS NULL OR academic_year=?) AND status IN ('initialized','pending') ORDER BY academic_year DESC,term_number ASC", [schoolId, academicYear, academicYear]); } else { const rows = [...(db.subscriptions || [])].filter(row => String(row.schoolId || row.school_id) === String(schoolId) && String(row.schoolType || row.school_type || '').toLowerCase() === 'private' && (!academicYear || String(row.academicYear || row.academic_year) === academicYear)); subscriptions = rows; intents = (db.paymentIntents || []).filter(row => String(row.schoolId || row.school_id) === String(schoolId) && String(row.schoolType || row.school_type || '').toLowerCase() === 'private' && (!academicYear || String(row.academicYear || row.academic_year) === academicYear) && ['initialized','pending'].includes(String(row.status || '').toLowerCase())); } const years = [...new Set([...subscriptions, ...intents].map(row => String(row.academicYear || row.academic_year || '')).filter(Boolean))].sort().reverse(); const selectedYear = academicYear || years[0] || null; const context = selectedYear ? await privateSubscriptionCycleState({ ...((relational.isConfigured() ? {} : { db })), schoolId }, selectedYear) : { count: 0, privateSubscriptionCountForAcademicYear: 0 }; const rows = [...subscriptions, ...intents].filter(row => !selectedYear || String(row.academicYear || row.academic_year) === selectedYear).sort((a,b) => Number(a.termNumber || a.term_number || 0) - Number(b.termNumber || b.term_number || 0)); const activeStudents = relational.isConfigured() ? Number((await relational.domainRows("SELECT COUNT(*) AS count FROM students WHERE school_id=? AND UPPER(status) IN ('ACTIVE','ADMITTED')", [schoolId]))[0]?.count || 0) : (db.students || []).filter(row => String(row.schoolId || row.school_id) === String(schoolId) && ['ACTIVE','ADMITTED'].includes(String(row.status || '').toUpperCase())).length; return json(res, 200, { dashboard: { academicYear: selectedYear, currentTerm: context.count ? Math.min(context.count, 3) : 1, privateSubscriptionCountForAcademicYear: Math.min(Number(context.privateSubscriptionCountForAcademicYear || context.count || 0), 3), cyclesRemaining: Math.max(0, 3 - Number(context.privateSubscriptionCountForAcademicYear || context.count || 0)), activeStudents, subscriptions: rows.map(row => ({ id: row.id, academicYear: row.academicYear || row.academic_year, termNumber: Number(row.termNumber || row.term_number || 0), subscriptionLabel: `Subscription ${Number(row.termNumber || row.term_number || 0)}/3`, reopeningDate: row.termStartDate || row.term_start_date || row.privateReopeningDate || row.private_reopening_date || null, vacationDate: row.termEndDate || row.term_end_date || row.privateVacationDate || row.private_vacation_date || null, durationDays: row.durationDays || row.duration_days || (row.termStartDate && row.termEndDate ? Math.round((new Date(row.termEndDate) - new Date(row.termStartDate)) / 86400000) + 1 : null), activeStudents: Number(row.activeStudentCountAtSubscription || row.active_student_count_at_subscription || row.activeStudentCount || row.active_student_count || activeStudents), amountDue: Number(row.subscriptionAmount || row.subscription_amount || 0), status: row.status || 'UNKNOWN' })) } }); } catch (error) { return domainErrorResponse(res, error); } }
  ...
  if (req.method === 'GET' && req.url.split('?')[0] === '/api/subscriptions/carry-forward/dashboard') { const query = new URL(req.url, 'http://edutrack.local').searchParams; const schoolId = validateText(query.get('schoolId'), { required: true, max: 80, pattern: /^[A-Za-z0-9._-]+$/ }); if (!schoolId) return json(res, 400, { error: 'schoolId is required' }); const auth = await authorize(req, res, db, { permission: 'reporting.read', scope: { schoolId } }); if (!auth) return; try { const schoolType = await persistedSchoolType(schoolId, db); if (!schoolType) return json(res, 404, { error: "School not found" }); let dashboard; if (relational.isConfigured()) { const row = await relational.getCarryForwardDashboard(schoolId, query.get('previousSubscriptionId') || null); if (row) { const population = Number(row.next_subscription_population || row.previous_end_population || 0); const pricing = subscriptionPolicy.calculateSubscriptionAmount(population, schoolType); dashboard = { previousSubscriptionPopulation: Number(row.previous_baseline_population || 0), verifiedAdditionalStudents: Number(row.verified_carry_forward || 0), carryForwardPopulation: Number(row.verified_carry_forward || 0), currentActivePopulation: population, nextSubscriptionBillablePopulation: population, pricePerStudentGhs: pricing.pricePerStudentGhs, amountDueGhs: pricing.amountGhs, currency: 'GHS', carryForward: publicCarryForward(row) }; } else dashboard = compatibilityCarryForwardDashboard(db, schoolId, query.get('previousSubscriptionId') || null, schoolType); } else dashboard = compatibilityCarryForwardDashboard(db, schoolId, query.get('previousSubscriptionId') || null, schoolType); return json(res, 200, { dashboard }); } catch (error) { return domainErrorResponse(res, error); } }
  if (req.method === 'GET' && req.url.split('?')[0] === '/api/subscriptions/population-dashboard') {
    const query = new URL(req.url, 'http://edutrack.local').searchParams; const schoolId = validateText(query.get('schoolId'), { required: true, max: 80, pattern: /^[A-Za-z0-9._-]+$/ }); const classId = query.get('classId') || null; const subscriptionId = query.get('subscriptionId') || null; if (!schoolId) return json(res, 400, { error: 'schoolId is required' }); const auth = await authorize(req, res, db, { permission: 'reporting.read', scope: { schoolId, classId } }); if (!auth) return; if (!classId && String(actorRole(auth) || '').toUpperCase() === 'TEACHER') return json(res, 403, { error: 'Teachers must request an assigned class dashboard' }); try { const schoolType = await persistedSchoolType(schoolId, db); if (!schoolType) return json(res, 404, { error: "School not found" }); let dashboard = relational.isConfigured() ? await relational.getPopulationDashboard(schoolId, subscriptionId) : compatibilityPopulationDashboard(db, schoolId, subscriptionId, classId); dashboard = decoratePopulationDashboard(dashboard, schoolType); if (classId && dashboard.classes) { dashboard.classes = dashboard.classes.filter(row => String(row.classId) === String(classId)); dashboard.totals = decoratePopulationDashboard({ classes: dashboard.classes, summary: dashboard.summary }, schoolType).totals; } const permissions = auth?.authorization?.permissions || []; const canViewFinancial = permissions.includes('*') || permissions.includes('payments.manage') || ['DEVELOPER_ROOT','SUPER_ADMIN','SCHOOL_ACCOUNTANT','ACCOUNTANT','HEADTEACHER'].includes(String(actorRole(auth) || '').toUpperCase()); if (!canViewFinancial) { dashboard.notification = { ...dashboard.notification, nextSubscriptionAmountGhs: null, carryForwardStudents: null }; dashboard.summary = { ...dashboard.summary, nextSubscriptionAmountGhs: null, carryForwardStudents: null }; } return json(res, 200, { dashboard }); } catch (error) { return json(res, 500, { error: 'Unable to retrieve population dashboard' }); }
  }
  ...
  if (req.method === 'GET' && req.url === '/api/subscriptions/plans') {
    return json(res, 200, { policyVersion: subscriptionPolicy.POLICY_VERSION, pricing: { schoolTypeRatesGhs: subscriptionPolicy.SCHOOL_TYPE_RATES, currency: subscriptionPolicy.CURRENCY, billingPeriod: subscriptionPolicy.BILLING_PERIOD }, plans: Object.values(subscriptionPolicy.PLANS).map(plan => ({ id: plan.id, name: plan.name, pricePerStudentGhs: plan.pricePerStudentGhs, currency: plan.currency, billingPeriod: plan.billingPeriod, capacity: plan.capacity, firstTermFree: plan.firstTermFree, smsIncluded: plan.smsIncluded, smsAddOn: plan.smsAddOn, termCalendar: plan.termCalendar })) });
  }
  ...
  if (req.method === 'POST' && req.url === '/api/subscriptions/quote') {
    ...
    try {
      ...
      return json(res, 200, { ...quote, schoolType, termStartDate: term.startDate, termEndDate: term.endDate, governmentTermReference: term.governmentCalendarVersion || term.governmentTermReference || null, subscriptionSequence, carryForward: publicCarryForward(carryForward) });
  ...
  if (req.method === 'POST' && req.url === '/api/subscriptions/claim-first-term-free') {
    ...
    try {
      ...
      const pricing = subscriptionPolicy.calculateSubscriptionAmount(activeStudentCount, 'government'); const now = new Date(); const dates = { startDate: term.startDate, endDate: term.endDate, durationDays: term.durationDays };
  ...
  if (req.method === 'POST' && req.url === '/api/auth/super-admin-login') {
    ...
    const user = db.users.find(item => item.active && String(item.credentialStatus || 'ACTIVE').toUpperCase() === 'ACTIVE' && String(item.role || '').toUpperCase() === 'SUPER_ADMIN' && String(item.email || '').toLowerCase() === String(identifier || '').toLowerCase() && (!item.developmentFixture || DEV_ACCESS_ENABLED));
  ...
  if (req.method === 'POST' && req.url === '/api/auth/login') {
    ...
    if (ipState.blocked || accountState.blocked) return json(res, 429, { error: GENERIC_AUTH_ERROR, retryAfter: Math.max(ipState.retryAfter || 0, accountState.retryAfter || 0) }, { 'Retry-After': String(Math.max(ipState.retryAfter || 1, accountState.retryAfter || 1)) });
    const user = db.users.find(u => u.active && String(u.credentialStatus || 'ACTIVE').toUpperCase() === 'ACTIVE' && String(u.email || '').toLowerCase() === identifier && (!u.developmentFixture || DEV_ACCESS_ENABLED));
    const locked = user && user.lockedUntil && new Date(user.lockedUntil) > new Date();
    const valid = !!user && String(user.credentialStatus || 'ACTIVE').toUpperCase() === 'ACTIVE' && !locked && verifyPassword(password, user.passwordHash) && verifyPassword(accessCode, user.accessCodeHash);
    if (!valid) {
  ...
  if (req.method === 'POST' && req.url === '/api/auth/credential-recovery/initiate') {
    if (!requireSameOrigin(req, res)) return;
    const auth = authUser(req, db); if (!auth) return json(res, 401, { error: GENERIC_AUTH_ERROR });
    const key = limitKey(req, 'credential-recovery'); const state = checkLimit(key, RESET_LIMIT);
    if (state.blocked) return json(res, 429, { error: 'Credential recovery could not be initiated' }, { 'Retry-After': String(state.retryAfter) });
    let input; try { input = await body(req); } catch { return json(res, 400, { error: 'Invalid recovery request' }); }
    const targetUserId = validateText(input.userId, { required: true, max: 80, pattern: /^[A-Za-z0-9._-]+$/ });
    if (!targetUserId) return json(res, 400, { error: 'Invalid recovery request' });
    const target = relational.isConfigured() ? await relational.findUserById(targetUserId) : db.users.find(user => user.id === targetUserId && user.active);
    const actorRole = String(auth.user.role || '').toUpperCase(); const targetRole = String(target?.role || '').toUpperCase();
    const allowed = actorRole === 'DEVELOPER_ROOT' || (actorRole === 'SUPER_ADMIN' && targetRole !== 'SUPER_ADMIN' && targetRole !== 'DEVELOPER_ROOT');
    if (!target || !allowed || targetRole === 'DEVELOPER_ROOT') return json(res, 403, { error: 'Credential recovery is not authorized' });
    registerFailure(key, RESET_LIMIT);
    const raw = randomToken(); const reset = { id: id('rst'), userId: target.id, tokenHash: tokenHash(raw), createdAt: new Date().toISOString(), expiresAt: new Date(Date.now() + RESET_TTL_MS).toISOString(), usedAt: null };
    if (relational.isConfigured()) await relational.createPasswordReset(reset); else { db.passwordResets.push(reset); saveDb(db); }
    if (relational.isConfigured()) await relational.appendAudit({ id: id('audit'), action: 'CREDENTIAL_RECOVERY_INITIATED', userId: auth.user.id, at: new Date().toISOString(), ip: clientIp(req), targetUserId: target.id, targetRole });
    else { db.audit.push({ id: id('audit'), userId: auth.user.id, action: 'CREDENTIAL_RECOVERY_INITIATED', at: new Date().toISOString(), targetUserId: target.id, targetRole }); saveDb(db); }
    return json(res, 201, { recoveryToken: raw, expiresAt: reset.expiresAt });
  }
  if (req.method === 'POST' && req.url === '/api/auth/password-reset/request') {
    ...
    registerFailure(key, RESET_LIMIT);
    const identifier = validateText(input.email, { required: true, max: 254, pattern: /^[^\s@]+@[^\s@]+\.[^\s@]+$/ }); const user = identifier && (relational.isConfigured() ? await relational.findUser(identifier) : db.users.find(u => u.active && String(u.email || '').toLowerCase() === identifier));
  ...
  if (req.method === 'POST' && req.url === '/api/auth/password-reset/confirm') {
    if (!requireSameOrigin(req, res)) return;
    let input; try { input = await body(req); } catch { return json(res, 400, { error: 'Invalid request' }); }
    const resetToken = validateText(input.token, { required: true, max: 512 }); const password = validateText(input.password, { required: true, max: 256 }) || ''; const accessCode = validateText(input.accessCode, { required: false, max: 256 }) || ''; const record = relational.isConfigured() ? await relational.findPasswordReset(tokenHash(resetToken || '')) : resetRecord(db, resetToken || ''); if (!record || password.length < 12) return json(res, 400, { error: 'Password reset could not be completed' }); const user = relational.isConfigured() ? await relational.findUserById(record.user_id) : db.users.find(u => u.id === record.userId && u.active); const requiresCredentialEstablishment = String(user?.credentialStatus || 'ACTIVE').toUpperCase() === 'RESET_REQUIRED'; if (!user || (requiresCredentialEstablishment && accessCode.length < 12)) return json(res, 400, { error: 'Password reset could not be completed' }); const nextHash = hashPassword(password); const nextAccessCodeHash = requiresCredentialEstablishment ? hashPassword(accessCode) : user.accessCodeHash; if (relational.isConfigured()) { if (!(await relational.consumePasswordReset(record.id))) return json(res,400,{error:'Password reset could not be completed'}); await relational.updateCredentialHashes(user.id,nextHash,nextAccessCodeHash); await relational.appendAudit({id:id('audit'),action:'PASSWORD_RESET_COMPLETED',userId:user.id,at:new Date().toISOString(),ip:clientIp(req),credentialEstablished:requiresCredentialEstablishment}); } else { user.passwordHash = nextHash; user.accessCodeHash = nextAccessCodeHash; user.credentialStatus = 'ACTIVE'; user.failedLoginCount = 0; user.lockedUntil = null; record.usedAt = new Date().toISOString(); invalidateSessions(db, user.id); db.audit.push({ id: id('audit'), userId: user.id, action: 'PASSWORD_RESET_COMPLETED', at: new Date().toISOString(), credentialEstablished:requiresCredentialEstablishment }); saveDb(db); } return json(res, 200, { ok: true });
  }
  ...
  if (req.method === 'GET' && req.url.split('?')[0] === '/api/signatures/active') {
    const q=new URL(req.url,'http://edutrack.local').searchParams; const input={schoolId:q.get('schoolId'),ownerType:q.get('ownerType'),staffId:q.get('staffId'),classId:q.get('classId')};
    if(!input.schoolId||!['HEADTEACHER','CLASS_TEACHER'].includes(input.ownerType)||(input.ownerType==='CLASS_TEACHER'&&(!input.staffId||!input.classId)))return json(res,400,{error:'Invalid signature scope'});
  ...
  if (req.method === 'POST' && req.url.split('?')[0] === '/api/signatures') {
    ...
    const auth=await authorize(req,res,db,{permission:'signature.manage',scope:{schoolId:input.schoolId}});if(!auth)return;
    try{if(input.ownerType==='HEADTEACHER'){const rows=await relational.academicRows('SELECT st.id FROM staff st JOIN staff_school_assignments a ON a.staff_id=st.id AND a.school_id=? AND a.active=TRUE JOIN user_roles ur ON ur.user_id=st.user_id JOIN roles ro ON ro.id=ur.role_id AND ro.name=\'HEADTEACHER\' WHERE st.school_id=? AND st.status=\'ACTIVE\' LIMIT 1',[input.schoolId,input.schoolId]);if(!rows.length)throw Object.assign(new Error('Active headteacher is not assigned to this school'),{code:'AUTHORIZATION_DENIED'});}else{const rows=await relational.academicRows('SELECT 1 FROM teacher_class_assignments t JOIN staff st ON st.id=t.staff_id WHERE t.staff_id=? AND t.class_id=? AND t.active=TRUE AND st.school_id=? AND st.status=\'ACTIVE\' LIMIT 1',[input.staffId,input.classId,input.schoolId]);if(!rows.length)throw Object.assign(new Error('Teacher is not assigned to this class'),{code:'AUTHORIZATION_DENIED'});}const row=await relational.saveSignature(input,auth.user.id);await auditDomainMutation(auth,'SIGNATURE_SAVED',req,{signatureId:row.id,schoolId:input.schoolId,ownerType:input.ownerType});return json(res,201,{signature:row});}catch(e){return domainErrorResponse(res,e)}
  }
  if (req.method === 'PATCH' && /^\/api\/signatures\/[A-Za-z0-9_-]+\/deactivate$/.test(req.url.split('?')[0])) {
    if(!requireSameOrigin(req,res))return; const idValue=req.url.split('/')[3]; let input;try{input=canonicalDomainPayload(await body(req));requireFields(input,['schoolId']);}catch(e){return domainErrorResponse(res,e)}const auth=await authorize(req,res,db,{permission:'signature.manage',scope:{schoolId:input.schoolId}});if(!auth)return;try{const changed=await relational.deactivateSignature(idValue,input.schoolId);if(!changed)return json(res,404,{error:'Active signature not found'});await auditDomainMutation(auth,'SIGNATURE_DEACTIVATED',req,{signatureId:idValue,schoolId:input.schoolId});return json(res,200,{deactivated:true});}catch(e){return domainErrorResponse(res,e)}
  }
  ...
  if (req.method === 'POST' && req.url === '/api/payments/paystack/initialize') {
    ...
    else { db.paymentIntents.push({ ...intentInput, school_id: context.schoolId, schoolType, school_type: schoolType, term_id: term.termId, governmentTermReference: term.governmentCalendarVersion || term.governmentTermReference || null, government_term_reference: term.governmentTermReference || null, privateReopeningDate: term.startDate || null, privateVacationDate: term.endDate || null, private_reopening_date: term.startDate || null, private_vacation_date: term.endDate || null, subscriptionSequence, subscription_sequence: subscriptionSequence, active_student_count: pricing.activeStudentCount, price_per_student: pricing.pricePerStudentGhs, subscription_amount: pricing.amountGhs, economic_value: pricing.amountGhs, createdAt: new Date().toISOString() }); saveDb(db); }
  ...
  if (req.method === 'POST' && req.url === '/api/payments/initialize') {
    ...
    const { term, subscriptionSequence } = paymentContext;
    const carryForward = await calculateCarryForwardForSchool(db, { schoolId: context.schoolId, previousSubscriptionId: input.previousSubscriptionId || null }, auth.user.id);
    const pricing = subscriptionPolicy.calculateSubscriptionAmount(context.activeStudentCount, schoolType); const dates = { startDate: term.startDate, endDate: term.endDate, durationDays: term.durationDays }; const reference = `EDU_${randomToken(18)}`;
