...
function admissionLevelForClassName(name) { const value = String(name || '').toUpperCase().replace(/\s+/g, ' ').trim(); if (/^KG\s*[0-9]/.test(value)) return 'KG'; if (/^(BASIC|CLASS)\s*[1-3]/.test(value)) return 'LOWER_PRIMARY'; if (/^(BASIC|CLASS)\s*[4-6]/.test(value)) return 'UPPER_PRIMARY'; if (/^JHS\s*[1-3]/.test(value)) return 'JHS'; return ''; }
  function assertProductionConfiguration() { if (process.env.NODE_ENV !== 'production') return; const failedChecks = []; if (!relational.isConfigured()) failedChecks.push('relational_database_url'); if (process.env.EDUTRACK_ENABLE_DEV_ACCESS === 'true') failedChecks.push('development_access_disabled'); if (!ALLOWED_ORIGINS.size || ALLOWED_ORIGINS.has('*') || [...ALLOWED_ORIGINS].some(origin => { try { const parsed = new URL(origin); return !['https:'].includes(parsed.protocol) || !parsed.hostname; } catch { return true; } })) failedChecks.push('allowed_origins_https'); if (!process.env.PAYSTACK_SECRET_KEY) failedChecks.push('paystack_secret_present'); if (!process.env.PAYSTACK_WEBHOOK_SECRET) failedChecks.push('paystack_webhook_secret_present'); const storageMode = privateStorage.mode(); if (storageMode !== 's3') failedChecks.push('storage_mode_s3'); if (storageMode === 's3' && !process.env.EDUTRACK_STORAGE_BUCKET) failedChecks.push('storage_bucket_present_when_s3'); if (failedChecks.length) { console.warn('[TEMP production-config diagnostic] failed checks:', failedChecks.join(', ')); throw new Error('Production configuration is incomplete'); } privateStorage.validate(); }
