...
async function run(){
  ...
  try{
    ...
    try{
      ...
      const stored=JSON.parse(fs.readFileSync(DB_FILE,'utf8')); const intent=stored.paymentIntents.find(x=>x.idempotencyKey==='gov-official'); assert.equal(intent.amount,1000);
