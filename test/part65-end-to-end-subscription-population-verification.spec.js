...
async function run() {
  ...
  try {
    ...
    try {
      ...
      const plans = await request('/api/subscriptions/plans'); assert.equal(plans.status, 200); const catalog = await plans.json(); assert.deepEqual(catalog.pricing.schoolTypeRatesGhs, { government: 2, private: 5 }); assert.equal(catalog.pricing.currency, 'GHS'); assert.equal(catalog.pricing.billingPeriod, 'term');
      ...
      assert.equal(dashboard.summary.subscriptionPopulation, 300); assert.equal(dashboard.summary.currentActivePopulation, 325); assert.equal(dashboard.summary.netAdditionalStudents, 25); assert.equal(dashboard.summary.carryForwardStudents, 25); assert.equal(dashboard.summary.nextSubscriptionAmountGhs, 1625); assert.equal(dashboard.notification.message, '25 additional active students detected.'); assert.equal(dashboard.reportCardCheck.status, 'matched'); assert.equal(dashboard.reportCardCheck.difference, 0);
      ...
      response = await request(`/api/subscriptions/population-dashboard?schoolId=${schoolId}&subscriptionId=sub65-2`, { headers }); assert.equal(response.status, 200); dashboard = (await response.json()).dashboard; assert.equal(dashboard.summary.currentActivePopulation, 350); assert.equal(dashboard.summary.netAdditionalStudents, 25); assert.equal(dashboard.summary.carryForwardStudents, 25); assert.equal(dashboard.summary.nextSubscriptionAmountGhs, 1750); assert.equal(dashboard.notification.message, '25 additional active students detected.');
      payment = await request('/api/payments/initialize', { method: 'POST', headers: { ...headers, 'x-idempotency-key': 'part65-new-year-350' }, body: JSON.stringify({ schoolId, schoolType: 'private', planId: 'private', academicYear: '2027/2028', termNumber: 1, reopeningDate: '2027-09-01', closingDate: '2027-12-31', amount: 1, currency: 'GHS' }) }); const newYearText = await payment.text(); assert.equal(payment.status, 201, newYearText); assert.equal(JSON.parse(newYearText).amountGhs, 1750); assert.equal(JSON.parse(newYearText).pricePerStudentGhs, 5);
      ...
      response = await request('/api/payments/paystack/initialize', { method: 'POST', headers: { ...headers, 'x-idempotency-key': 'part65-paystack' }, body: JSON.stringify({ schoolId, schoolType: 'private', planId: 'private', academicYear: '2028/2029', termNumber: 1, reopeningDate: '2028-09-01', closingDate: '2028-12-31', email: EMAIL }) }); const paystackText = await response.text(); assert.equal(response.status, 503, paystackText);
