...
const remoteValidator = html.match(/async function v43ValidateSchoolLoginRemote[\s\S]*?function v43ValidateSchoolLogin\(/);
...
assert.match(deterministicFix[1], /completeHandoff/, 'school login must retry if the shell is still loading');
assert.match(deterministicFix[1], /setTimeout\(completeHandoff, 25\)/, 'school login must retry the shell handoff while it initializes');
assert.match(deterministicFix[1], /Date\.now\(\) - loginStartedAt >= 9800/, 'dashboard handoff must have a hard ten-second deadline');
assert.match(deterministicFix[1], /restoreLoginButton\(\)/, 'all School login outcomes must restore the login button');
assert.match(deterministicFix[1], /School authentication service is unavailable/, 'network failures must show a service error');
